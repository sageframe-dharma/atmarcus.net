// hero-config.js — generated from author mode

window.HERO_COLORS = {
  ink:        '#1a1a1a',
  inkLight:   '#4a4a4a',
  inkMuted:   '#7a7a7a',
  ground:     '#f5f2ed',
  groundWarm: '#ebe6dd',
  accent:     '#c45a2d',
  umber:      '#6b3a2a',
  rule:       '#d0c9be',
  plum:       '#8b5e8b',
  ochre:      '#b8860b',
  steel:      '#3a7ca5',
  sage:       '#5a8a3a',
  clay:       '#a0846b',
};

window.HERO_PROJECTS = [
  // Current work — research & methodology
  { name: "Ho System", url: "https://atmarcus.net/work/ho-system", color: HERO_COLORS.accent, weight: 10 },
  { name: "Research Program", url: "https://atmarcus.net/work/research", color: HERO_COLORS.accent, weight: 9 },
  { name: "Constructive Interference", url: "https://sageframe.substack.com", color: HERO_COLORS.accent, weight: 10 },
  { name: "Destructive Interference", url: "https://atmarcus.net/work/research#framework", color: HERO_COLORS.plum, weight: 7 },
  { name: "Sageframe", url: "https://sageframe.net", color: HERO_COLORS.plum, weight: 5 },
  { name: "Sageframe Knowledge System", url: "https://atmarcus.net/work/sageframe", color: HERO_COLORS.plum, weight: 7 },

  // Shipped production systems
  { name: "Glassroom", url: "https://glassroom.sageframe.net", color: HERO_COLORS.steel, weight: 8 },
  { name: "Kanyō", url: "https://kanyo.sageframe.net", color: HERO_COLORS.sage, weight: 8 },
  { name: "m4Bookmaker", url: "https://m4bookmaker.sageframe.net", color: HERO_COLORS.ochre, weight: 5 },
  { name: "Hōzō Sentinel", url: "https://github.com/sageframe-no-kaji/hozo-sentinel", color: HERO_COLORS.steel, weight: 2 },
  { name: "SSH, Actually", url: "https://github.com/sageframe-no-kaji/ssh-actually", color: HERO_COLORS.ink, weight: 3 },
  { name: "pptx-builder", url: "https://pptx.sageframe.net", color: HERO_COLORS.ink, weight: 1 },
  { name: "audiobook-qc", url: "https://github.com/sageframe-no-kaji/audiobook-qc", color: HERO_COLORS.ink, weight: 1 },

  // Consulting
  { name: "Blueprint for Creativity", url: "https://atmarcus.net/#consulting", color: HERO_COLORS.clay, weight: 6 },

  // Organizational case studies
  { name: "NuVu SMS", url: "https://atmarcus.net/work/sms-case-study.html", color: HERO_COLORS.ochre, weight: 5 },
  { name: "CS Learning Framework", url: "https://atmarcus.net/work/citizen-schools.html", color: HERO_COLORS.sage, weight: 5 },
  { name: "ARCK System Design", url: "https://atmarcus.net/work/arck.html", color: HERO_COLORS.clay, weight: 4 },
];

window.HERO_RES_TUNE = {
  "speed": 0.3,
  "wavelength": 66,
  "drift": 0.32,
  "smoothing": 0.55,
  "fadeBase": 14
};
